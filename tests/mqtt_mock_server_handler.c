...
static void mqtt_mock_server_wait_for_count(
    struct aws_channel_handler *handler,
    size_t count,
    int64_t time_to_wait,
    bool (*s_is_count_complete)(void *)) {
    struct mqtt_mock_server_handler *testing_handler = handler->impl;

    struct count_waiter waiter;
    waiter.testing_handler = testing_handler;
    waiter.wait_for_count = count;

    aws_mutex_lock(&testing_handler->lock);
    aws_condition_variable_wait_for_pred(
        &testing_handler->cvar, &testing_handler->lock, time_to_wait, s_is_count_complete, &waiter);
    aws_mutex_unlock(&testing_handler->lock);
}

static bool s_is_pubacks_complete(void *arg) {
    struct count_waiter *waiter = arg;
...
void mqtt_mock_server_wait_for_pubacks(struct aws_channel_handler *handler, size_t puback_count, int64_t time_to_wait) {
    mqtt_mock_server_wait_for_count(handler, puback_count, time_to_wait, s_is_pubacks_complete);
}

static bool s_is_publishes_received_complete(void *arg) {
    struct count_waiter *waiter = arg;

    return waiter->testing_handler->publishes_received >= waiter->wait_for_count;
}
...
void mqtt_mock_server_wait_for_publishes_received(
    ...
    int64_t time_to_wait) {
    mqtt_mock_server_wait_for_count(handler, publishes_received_count, time_to_wait, s_is_publishes_received_complete);
}

static bool s_is_unsubscribe_received_complete(void *arg) {
    struct count_waiter *waiter = arg;

    return waiter->testing_handler->unsubscribe_received >= waiter->wait_for_count;
}
...
void mqtt_mock_server_wait_for_unsubscribe_received(
    ...
    int64_t time_to_wait) {
    mqtt_mock_server_wait_for_count(
        handler, unsubscribe_received_count, time_to_wait, s_is_unsubscribe_received_complete);
}
